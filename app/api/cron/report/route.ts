...
export async function GET(request: Request) {
  ...
  const { data: report } = await supabase.from('view_monthly_pnl').select('*').limit(1).single();
  ...
  await sendAdminAlert(
    ...
    `Revenue: ${report.total_revenue} | Expenses: ${report.total_expenses} | Profit: ${report.net_profit}`
  );
