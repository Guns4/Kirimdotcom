# API Monetization & Metering Setup (PowerShell)

Write-Host "Initializing API Metering System..." -ForegroundColor Cyan
Write-Host "=================================================" -ForegroundColor Cyan

# 1. Database Schema
Write-Host "1. Generating SQL Schema: metering_schema.sql" -ForegroundColor Yellow
$schemaContent = @'
...
'@
$schemaContent | Set-Content -Path "metering_schema.sql" -Encoding UTF8
Write-Host "   [?] Schema created." -ForegroundColor Green

# 2. Helper Library
Write-Host "2. Creating Auth Helper: src/lib/api-auth.ts" -ForegroundColor Yellow
$dirLib = "src\lib"
...
'@
$authContent | Set-Content -Path "src\lib\api-auth.ts" -Encoding UTF8
Write-Host "   [?] Helper created." -ForegroundColor Green

# 3. Example Usage Route
Write-Host "3. Creating Example Route: src/app/api/v1/example/route.ts" -ForegroundColor Yellow
$dirApi = "src\app\api\v1\example"
...
'@
$routeContent | Set-Content -Path "src\app\api\v1\example\route.ts" -Encoding UTF8
Write-Host "   [?] Example route created." -ForegroundColor Green

Write-Host ""
Write-Host "=================================================" -ForegroundColor Cyan
Write-Host "API Metering Setup Complete!" -ForegroundColor Green
Write-Host "1. Run 'metering_schema.sql' in Supabase." -ForegroundColor White
Write-Host "2. Use 'authenticateApi(req, cost)' in your API Routes to protect them." -ForegroundColor White
Write-Host "3. Check 'src/app/api/v1/example/route.ts' for reference." -ForegroundColor White
