...
# =============================================================================
# Cross-Selling: Smart Recommendation Engine
# =============================================================================

echo "Initializing Recommendation Engine..."
echo "================================================="

# 1. Configuration (The Brain)
echo "1. Creating Rules: src/config/recommendation-rules.ts"
mkdir -p src/config
cat <<EOF > src/config/recommendation-rules.ts
...
export const RECOMMENDATION_RULES: RecommendationRule[] = [
  // 1. Electronics Cross-Sell
  {
    ...
    productImage: 'https://images.unsplash.com/photo-1592899677712-a170135c8688?w=300&h=300&fit=crop', // Placeholder
    link: 'https://shopee.co.id/search?keyword=casing%20hp', // Example Affiliate Link
...
export function getRecommendations(itemName: string): RecommendationRule[] {
  ...
  // If no specific match, return general advice or empty
  if (matches.length === 0) {
      // return [RECOMMENDATION_RULES.find(r => r.id === 'rec-general')!];
      return []; 
  }
...
# 2. UI Component
echo "2. Creating Widget: src/components/ads/SmartRecommendation.tsx"
mkdir -p src/components/ads
cat <<EOF > src/components/ads/SmartRecommendation.tsx
'use client';
...
echo ""
echo "================================================="
echo "Recommendation Engine Ready!"
echo "1. Rules defined in 'src/config/recommendation-rules.ts'."
echo "2. Use <SmartRecommendation itemName={tracking.goods_name} /> in your result page."
