...
# setup-bunker-backup.sh
# ----------------------
# Disaster Recovery Script: Dump -> Encrypt -> Immutable Upload
# Runs on Linux Server (Cron Job)

echo "🛡️  Setting up Bunker Backup Script..."

mkdir -p scripts/ops

cat > scripts/ops/disaster-recovery.sh << 'EOF'
#!/bin/bash
...
chmod +x scripts/ops/disaster-recovery.sh

echo "✅ Operations Script Created: scripts/ops/disaster-recovery.sh"
echo "👉 Configure DB_URL and S3_BUCKET inside the script before adding to Crontab."
