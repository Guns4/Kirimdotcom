...
-- Fixed: Changed public.users to auth.users for Supabase compatibility
-- ============================================
...
DROP POLICY IF EXISTS "Users can view their own RFM segment" ON public.marketing_rfm_segments;
DROP POLICY IF EXISTS "Service role can manage RFM segments" ON public.marketing_rfm_segments;
DROP POLICY IF EXISTS "Admins can view AB tests" ON public.marketing_ab_tests;
DROP POLICY IF EXISTS "Service role can manage AB tests" ON public.marketing_ab_tests;
DROP POLICY IF EXISTS "Service role can manage automations" ON public.marketing_automations;
DROP POLICY IF EXISTS "Users can view their own campaign logs" ON public.marketing_campaign_logs;
DROP POLICY IF EXISTS "Service role can manage campaign logs" ON public.marketing_campaign_logs;
CREATE POLICY "Users can view their own RFM segment" ON public.marketing_rfm_segments FOR
SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Service role can manage RFM segments" ON public.marketing_rfm_segments FOR ALL TO service_role USING (true);
CREATE POLICY "Admins can view AB tests" ON public.marketing_ab_tests FOR
SELECT TO authenticated USING (true);
CREATE POLICY "Service role can manage AB tests" ON public.marketing_ab_tests FOR ALL TO service_role USING (true);
CREATE POLICY "Service role can manage automations" ON public.marketing_automations FOR ALL TO service_role USING (true);
CREATE POLICY "Users can view their own campaign logs" ON public.marketing_campaign_logs FOR
SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Service role can manage campaign logs" ON public.marketing_campaign_logs FOR ALL TO service_role USING (true);
...
CREATE OR REPLACE FUNCTION calculate_rfm_segments() RETURNS TABLE (
        ...
        frequency_score INT,
        monetary_score INT
    ) AS $$ BEGIN RETURN QUERY WITH user_stats AS (
...
COMMENT ON TABLE public.marketing_campaign_logs IS 'Logs of sent marketing campaigns per user';