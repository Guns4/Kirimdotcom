...
# =============================================================================
# Setup Social Content Gen V2 (Phase 114)
# Automated IG Feed Creation with Custom Input
# =============================================================================

echo "Setting up Social Content Studio V2..."


# 1. Data Source (JSON Tips)
echo "1. Creating Data Source: src/data/social-tips.json"
mkdir -p src/data

cat <<EOF > src/data/social-tips.json
[
  {
    "id": 1,
    "category": "TIPS KIRIM",
    "title": "Jangan Kirim Powerbank via Udara!",
    "content": "Baterai Lithium dianggap Dangerous Goods di pesawat. Gunakan kargo darat jika ingin kirim powerbank agar tidak diretur bandara."
  },
  {
    "id": 2,
    "category": "PACKING HACK",
    "title": "Rumus Packing Aman: 3 Lapis",
    "content": "1. Bubble Wrap tebal.\\n2. Kardus Corrugated keras.\\n3. Lakban keliling penuh (H method). Anti bonyok!"
  },
  {
    "id": 3,
    "category": "HEMAT ONGKIR",
    "title": "Kirim Banyak? Pakai Cargo!",
    "content": "Untuk paket di atas 10kg, JNE Trucking (JTR) atau J&T Cargo bisa hemat ongkir hingga 70% dibanding reguler."
  }
]
EOF
echo "   [?] Tips database created."
echo ""

# 2. Page & Component
echo "2. Creating Studio: src/app/dashboard/admin/social-studio/page.tsx"
mkdir -p src/app/dashboard/admin/social-studio

cat <<EOF > src/app/dashboard/admin/social-studio/page.tsx
'use client';
                            ...
                            {(Object.keys(themes) as Array<keyof typeof themes>).map((t) => (
                                <button
                                    ...
                                    className={\`h-10 rounded-lg transition-all border-2 \${
                                        theme === t ? 'border-indigo-600 scale-105' : 'border-transparent'
                                    } \${themes[t]}\`}
                                    title={t}
                                />
                            ))}
