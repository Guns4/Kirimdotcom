    ...
    <div className="max-w-xs mx-auto p-4 bg-white rounded-xl shadow-lg border">
      ...
      <div className="flex justify-center gap-2 mb-6">
        {[...Array(6)].map((_, i) => (
          <div
            key={i}
            className={`w-4 h-4 rounded-full border ${i < pin.length ? 'bg-blue-600 border-blue-600' : 'bg-gray-100 border-gray-300'}`}
          />
        ))}
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((n) => (
          <button
            ...
            className="p-4 text-xl font-semibold bg-gray-50 rounded-lg active:bg-gray-200"
          >
            {n}
          </button>
        ))}
        ...
        <button
          ...
          className="p-4 text-xl font-semibold text-red-500 bg-gray-50 rounded-lg active:bg-gray-200"
        >
          ⌫
        </button>
      </div>
