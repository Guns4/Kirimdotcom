...
CREATE TABLE IF NOT EXISTS digital_products (
    ...
    -- in bytes
    category TEXT,
    -- 'ebook', 'template', 'design', 'software'
    preview_image TEXT,
...
CREATE POLICY "Public read active products" ON digital_products FOR
SELECT USING (is_active = true);
-- Only product owners can update their products
CREATE POLICY "Owners manage products" ON digital_products FOR ALL USING (auth.uid() = created_by);
-- Users can view their own purchases
CREATE POLICY "Users view own purchases" ON user_purchases FOR
SELECT USING (auth.uid() = user_id);
-- Users can create purchases
CREATE POLICY "Users create purchases" ON user_purchases FOR
INSERT WITH CHECK (auth.uid() = user_id);
-- Users can view their download history
CREATE POLICY "Users view own downloads" ON product_downloads FOR
SELECT USING (auth.uid() = user_id);
...
AFTER
INSERT ON product_downloads FOR EACH ROW EXECUTE FUNCTION increment_download_count();