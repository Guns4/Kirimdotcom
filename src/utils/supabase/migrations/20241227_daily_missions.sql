...
-- ============================================================================
CREATE OR REPLACE FUNCTION calculate_checkin_reward(p_streak_day INTEGER)
RETURNS INTEGER AS $$
BEGIN
  -- Progressive rewards based on streak day
  CASE p_streak_day
    ...
    WHEN 7 THEN RETURN 100; -- Bonus on day 7
    ELSE RETURN 10; -- Reset cycle, same as day 1
  END CASE;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  SELECT last_check_in_date, check_in_streak, longest_streak
  ...
  IF v_new_streak > 7 THEN
    v_new_streak := ((v_new_streak - 1) % 7) + 1;
  END IF;
...
BEGIN
  ...
  v_can_checkin := (v_last_checkin IS NULL OR v_last_checkin < CURRENT_DATE);
  
  -- Calculate next reward
  IF v_can_checkin THEN
    IF v_last_checkin = CURRENT_DATE - INTERVAL '1 day' THEN
      v_next_reward := calculate_checkin_reward(v_current_streak + 1);
    ELSE
      v_next_reward := calculate_checkin_reward(1); -- Reset
    END IF;
  ELSE
    v_next_reward := 0;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'checked_in_today', v_last_checkin = CURRENT_DATE
  );
END;
$$ LANGUAGE plpgsql;
...
DO $$
BEGIN
  RAISE NOTICE '✅ Daily Missions System created successfully!';
