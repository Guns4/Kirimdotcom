...
export async function generateApiKey(name: string): Promise<{
    ...
    const { data, error } = await (supabase as any)
        ...
        .select('id, name, key_prefix, requests_count, requests_limit, scopes, is_active, created_at')
        .single()
...
export async function getApiKeys(): Promise<ApiKey[]> {
    ...
    const { data, error } = await (supabase as any)
        .from('api_keys')
        .select('id, name, key_prefix, requests_count, requests_limit, scopes, is_active, last_used_at, created_at')
        .eq('user_id', user.id)
...
export async function validateApiKeyServer(apiKey: string): Promise<{
    ...
    const { data, error } = await (supabase as any)
        .from('api_keys')
        .select('id, user_id, requests_count, requests_limit, is_active, expires_at')
        .eq('key_hash', keyHash)
        .single()
