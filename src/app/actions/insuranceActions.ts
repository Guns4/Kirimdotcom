...
export async function purchaseInsurance(
  ...
  try {
    ...
    const premiumAmount =
      Math.round(((declaredValue * policy.premium_rate) / 100) * 100) / 100;
...
export async function checkAutoClaimConditions(
  ...
  try {
    ...
    const { data: claimCheck } = await supabase.rpc(
      'check_auto_claim_conditions',
      {
    ...
    // Auto-create claim
    const { data: claimId } = await supabase.rpc('auto_create_claim', {
