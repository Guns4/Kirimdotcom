...
export async function saveWASettings(settings: {
  ...
  try {
    ...
    const { error } = await (supabase.from('wa_user_settings') as any).upsert({
      user_id: user.id,
      ...settings,
...
export async function updateWATemplate(
  ...
  try {
    ...
    const { error } = await (supabase.from('wa_templates') as any)
      .update({
...
export async function sendWANotification(
  ...
  try {
    ...
    const { data: settings } = await (supabase.from('wa_user_settings') as any)
    ...
    const { data: log } = await (supabase.from('wa_notification_log') as any)
      .select('message_sent')
      .eq('id', logId)
      .single();
    ...
    await (supabase.from('wa_notification_log') as any)
      .update({
  ...
  try {
    ...
    if (userId) {
      const { data: settings } = await (
        supabase.from('wa_user_settings') as any
      )
        .select('wa_api_key')
        .eq('user_id', userId)
        .single();
