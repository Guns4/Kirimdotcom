  ...
  return safeAction(
    async (input, user) => {
      ...
      if (newOrder.resi_number && newOrder.courier) {
        await refreshSingleOrderTracking(
          newOrder.id,
          newOrder.resi_number,
          newOrder.courier
        );
      }
...
export async function getOrders(): Promise<
  ActionResponse<{ data: Order[]; stats: any }>
> {
  return safeAction(
    async (_, user) => {
      if (!user) throw new Error('Unauthorized');
      ...
      const stats = orders.reduce(
        (acc, order) => {
          ...
          if (
            status === 'Returned' ||
            order.tracking_status.includes('RETUR')
          ) {
            acc.returned += 1;
          }
          return acc;
...
async function refreshSingleOrderTracking(
  orderId: string,
  resi: string,
  courier: string
) {
...
export async function refreshActiveOrders(): Promise<
