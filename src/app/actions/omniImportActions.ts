...
export async function updateImportBatch(
  ...
  try {
    ...
    if (stats.importedCount !== undefined)
      updateData.imported_count = stats.importedCount;
    if (stats.skippedCount !== undefined)
      updateData.skipped_count = stats.skippedCount;
    if (stats.errorCount !== undefined)
      updateData.error_count = stats.errorCount;
    ...
    await supabase.from('import_batches').update(updateData).eq('id', batchId);
  } catch (error) {
...
export async function getOrders(
  ...
  try {
    ...
    if (filters?.search) {
      query = query.or(
        `order_number.ilike.%${filters.search}%,customer_name.ilike.%${filters.search}%,awb_number.ilike.%${filters.search}%`
      );
    }
