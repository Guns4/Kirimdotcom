  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === 'PGRST116') {
        ...
        return {
          success: false,
          error: 'Kode pos tidak ditemukan di database kami.',
        };
      }
      throw error;
    }
...
export async function reportBuyer(
  phoneHash: string,
  reason: string
): Promise<ReportBuyerResult> {
  const supabase = (await createClient()) as any;
  ...
  try {
    ...
    // 1. Log the report
    const { error: logError } = await supabase.from('buyer_reports').insert({
    ...
    if (existing) {
      ...
      // Insert
      await supabase.from('reported_buyers').insert({
        phone_hash: phoneHash,
        report_count: 1,
        last_reported_at: new Date().toISOString(),
      });
