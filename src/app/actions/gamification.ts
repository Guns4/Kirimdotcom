...
// Rank definitions
export const RANKS = {
    SCOUT: { name: 'Scout', minXP: 0, maxXP: 50, icon: '🔰', color: 'text-gray-400', bgColor: 'bg-gray-600/20' },
    NAVIGATOR: { name: 'Navigator', minXP: 51, maxXP: 200, icon: '🧭', color: 'text-blue-400', bgColor: 'bg-blue-600/20' },
    MASTER: { name: 'Logistics Master', minXP: 201, maxXP: null, icon: '🏆', color: 'text-yellow-400', bgColor: 'bg-yellow-600/20' },
} as const
...
export function getProgressToNextRank(xp: number) {
    ...
    const nextRank = currentRank.name === 'Scout' ? RANKS.NAVIGATOR : RANKS.MASTER
    const xpInCurrentRank = xp - currentRank.minXP
    const xpNeededForNext = nextRank.minXP - currentRank.minXP
    const percent = Math.min(100, Math.round((xpInCurrentRank / xpNeededForNext) * 100))
