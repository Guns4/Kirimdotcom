...
export async function awardDailyLoginPoints(): Promise<PointsResult> {
    try {
        ...
        const { data: canEarn } = await supabase
            .rpc('can_earn_daily_points', {
                p_user_id: user.id,
 ...
 */
export async function awardTrackingPoints(resiNumber: string): Promise<PointsResult> {
    try {
        const supabase = await createClient();
 ...
 */
export async function awardSocialSharePoints(platform: string): Promise<PointsResult> {
    try {
        const supabase = await createClient();
