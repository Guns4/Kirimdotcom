...
export async function checkOngkir(
  ...
  try {
    ...
    if (cached && !customKey) {
      // Skip cache if custom key is used (to get fresh data)
          ...
          apiResponse.data.forEach((service, index) => {
            service.cost.forEach((cost) => {
              allRates.push({
                ...
                serviceType: service.service.includes('REG')
                  ? 'Regular'
                  : 'Express',
...
export async function trackResi(
  ...
  try {
    ...
    return {
      success: false,
      error: isSystemError
        ? 'Gangguan sistem pada server ekspedisi.'
        : getErrorMessage(error),
      errorType,
      courier: params.courierCode,
      officialUrl: params.courierCode
        ? getOfficialFallbackUrl(params.courierCode)
        : undefined,
    };
  }
}
