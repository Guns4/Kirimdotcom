...
export async function requestDisputeOtp(rawPhone: string): Promise<RequestOtpResult> {
    // 1. Validate phone
    ...
    return { success: true, message: 'OTP telah dikirim via WhatsApp/SMS (Cek Console: 123456)' }
}
...
export async function verifyDisputeOtp(rawPhone: string, otp: string): Promise<VerifyOtpResult> {
    const storedOtp = otpStore.get(rawPhone)

    if (storedOtp === otp) {
        otpStore.delete(rawPhone) // Clear used OTP
        return { success: true, token: `verified_${Date.now()}_${rawPhone.slice(-4)}` }
    }
...
export async function submitDispute(phoneHash: string, reason: string, contactInfo: string, proofUrl?: string): Promise<SubmitDisputeResult> {
    ...
    try {
        ...
        return { success: true, message: 'Permintaan pemutihan berhasil dikirim. Kami akan meninjau dalam 1x24 jam.' }
    } catch (error) {
...
export async function updateDisputeStatus(id: string, status: 'Approved' | 'Rejected', phoneHash: string) {
    const supabase = await createClient() as any
