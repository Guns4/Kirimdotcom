...
function analyzeSentiment(comment: string = '', rating: number): 'positive' | 'negative' | 'neutral' {
    ...
    const positiveKeywords = ['bagus', 'cepat', 'baik', 'mantap', 'recommended', 'aman', 'tepat waktu', 'puas', 'excellent', 'great']
    const negativeKeywords = ['lama', 'lambat', 'buruk', 'jelek', 'rusak', 'hilang', 'telat', 'kecewa', 'bad', 'slow']
...
export async function submitCourierReview(params: SubmitReviewParams): Promise<ReviewResult> {
    try {
...
export async function getCourierStatistics(period: 'week' | 'month' | 'all' = 'month'): Promise<CourierStats[]> {
    try {
        const cookieStore = cookies()
        const supabase = await createClient(cookieStore)
