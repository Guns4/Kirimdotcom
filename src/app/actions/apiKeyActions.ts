...
export async function generateAPIKey(keyName: string, isPaid: boolean = false) {
  try {
    ...
    return {
      ...
      message:
        'API key generated successfully! Save the secret - it cannot be recovered.',
    };
  } catch (error) {
...
export async function getUserAPIKeys() {
  try {
    ...
    const { data, error } = await supabase
      .from('api_keys')
      .select(
        'id, key_name, api_key, is_paid, is_active, rate_limit_per_day, total_requests, requests_today, created_at'
      )
      .eq('user_id', user.id)
