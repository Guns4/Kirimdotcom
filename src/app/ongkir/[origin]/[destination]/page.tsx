...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    return {
        ...
        keywords: [`ongkir ${origin} ${destination}`, `tarif pengiriman ${origin} ${destination}`, `ekspedisi ${origin} ${destination}`, `jne ${origin} ${destination}`],
    };
}
            ...
            <section className="bg-gradient-to-br from-indigo-900 to-blue-800 text-white py-20 px-4">
                <div className="max-w-4xl mx-auto text-center">
                    ...
                    <p className="text-xl text-blue-100 mb-8 max-w-2xl mx-auto">
                        Bandingkan tarif JNE, J&T, SiCepat, Shopee Express, dan Cargo.
                        Mulai <strong>Rp 8.000/kg</strong>.
                    </p>
                    <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 inline-flex gap-8 items-center border border-white/20">
                        <div className="text-left">
                            <div className="text-xs text-blue-300 uppercase tracking-widest">ASAL</div>
                            <div className="text-2xl font-bold">{o}</div>
                        </div>
                        <div className="text-3xl opacity-50">✈️</div>
                        <div className="text-left">
                            <div className="text-xs text-blue-300 uppercase tracking-widest">TUJUAN</div>
            ...
            <div className="max-w-5xl mx-auto px-4 py-12">
                ...
                <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden mb-12">
                    <div className="p-6 border-b bg-gray-50 flex items-center justify-between">
                        <h2 className="font-bold text-xl text-gray-800">Perbandingan Tarif (Estimasi)</h2>
                        <span className="text-xs font-medium bg-green-100 text-green-700 px-3 py-1 rounded-full">Updated Hari Ini</span>
                    </div>
                    ...
                    <table className="w-full">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            ...
                            {[
                                { name: 'JNE', service: 'REG', est: '2-3 Hari', price: 'Rp 12.000' },
                                { name: 'J&T', service: 'EZ', est: '2-3 Hari', price: 'Rp 12.000' },
                                { name: 'SiCepat', service: 'REG', est: '1-2 Hari', price: 'Rp 11.500' },
                                { name: 'JNE', service: 'JTR (Cargo)', est: '4-7 Hari', price: 'Rp 55.000 (Min 10kg)' },
                            ].map((row, i) => (
                                <tr key={i} className="hover:bg-gray-50 transition">
                                    <td className="px-6 py-4 font-bold text-gray-800">{row.name}</td>
                                    <td className="px-6 py-4 text-gray-600 font-medium">{row.service}</td>
                                    <td className="px-6 py-4 text-gray-500">{row.est}</td>
                                    <td className="px-6 py-4 text-right font-bold text-indigo-600">{row.price}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                    <div className="p-6 bg-gray-50 text-center">
                        <p className="text-sm text-gray-500 mb-4">*Harga di atas adalah estimasi. Cek harga real-time akurat sekarang.</p>
                        <Link
                            href="/cek-ongkir"
                ...
                <section className="mb-12">
                    <h2 className="text-2xl font-bold mb-6 text-gray-900">Pertanyaan yang Sering Diajukan</h2>
                    <div className="space-y-4">
                        <div className="bg-gray-50 p-6 rounded-xl">
                            <h3 className="font-bold text-lg mb-2">Berapa ongkir termurah dari {o} ke {d}?</h3>
                            <p className="text-gray-600">
                                Untuk paket kecil (dibawah 1kg), tarif termurah biasanya dimulai dari Rp 8.000 - Rp 12.000 menggunakan layanan ekonomi.
                                Untuk paket besar (di atas 10kg), layanan Kargo seperti JNE JTR atau J&T Cargo jauh lebih murah.
                            </p>
                        </div>
                        <div className="bg-gray-50 p-6 rounded-xl">
                            <h3 className="font-bold text-lg mb-2">Ekspedisi apa saja yang tersedia?</h3>
                            <p className="text-gray-600">
                                Kami menyediakan perbandingan tarif untuk JNE, J&T, SiCepat, Anteraja, Pos Indonesia, Lion Parcel, dan berbagai layanan kargo lainnya.
                            </p>
                        </div>
                    </div>
                </section>
                ...
                {/* Internal Linking */}
                <section>
                    <h3 className="text-lg font-bold mb-4 text-gray-800">Rute Populer Lainnya dari {o}</h3>
                    <div className="flex flex-wrap gap-2">
                        {cities.slice(0, 12).filter(c => c.toLowerCase() !== origin).map(city => (
                            <Link
                                key={city}
