...
export default function AdminDisputesPage() {
    ...
    const handleAction = async (id: string, status: 'Approved' | 'Rejected', phoneHash: string) => {
                ...
                <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-white/5">
                        {loading ? (
                            ...
                            <tr><td colSpan={6} className="p-8 text-center text-gray-500">No disputes found.</td></tr>
                        ) : (
                            disputes.map((d) => (
                                <tr key={d.id} className="text-sm hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="p-4 text-gray-500 font-mono text-xs max-w-[150px] truncate" title={d.phone_hash}>
                                    ...
                                    <td className="p-4">
                                        <span className={`px-2 py-1 rounded text-xs font-medium ${d.status === 'Approved' ? 'bg-green-500/20 text-green-400' :
                                    ...
                                    <td className="p-4">
                                        {d.status === 'Pending' && (
                                            <div className="flex gap-2">
                                                <button
                                                    onClick={() => handleAction(d.id, 'Approved', d.phone_hash)}
                                                ...
                                                <button
                                                    onClick={() => handleAction(d.id, 'Rejected', d.phone_hash)}
