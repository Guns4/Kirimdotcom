import { getUserInsurancePolicies, getUserClaims, purchaseInsurance, fileInsuranceClaim, formatRupiah } from '@/app/actions/insuranceActions';
...
export default async function InsuranceDashboardPage() {
    ...
    const handlePurchase = async (formData: FormData) => {
        ...
        await purchaseInsurance(trackingNumber, declaredValue, courier, destination);
    ...
    const handleFileClaim = async (formData: FormData) => {
        ...
        const claimType = formData.get('claim_type') as 'lost' | 'damaged' | 'delayed';
        ...
        <main className="min-h-screen bg-gray-50 py-8 px-4">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-gray-600">Lindungi paket Anda dari risiko hilang atau rusak</p>
                </div>
                    ...
                    <div className="bg-white rounded-xl shadow-md p-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-bold text-gray-900">{activeInsurance.length}</p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="grid lg:grid-cols-2 gap-8 mb-8">
                    {/* Purchase Insurance */}
                    <div className="bg-white rounded-xl shadow-md p-6">
                        <h2 className="text-xl font-bold text-gray-900 mb-6">Beli Asuransi Baru</h2>
                    ...
                    {/* File Claim */}
                    <div className="bg-white rounded-xl shadow-md p-6">
                        <h2 className="text-xl font-bold text-gray-900 mb-6">Ajukan Klaim</h2>

                        {activeInsurance.length === 0 ? (
                            <div className="text-center py-8">
                                <AlertCircle className="w-12 h-12 text-gray-300 mx-auto mb-3" />
                                <p className="text-gray-500">Tidak ada asuransi aktif</p>
                                <p className="text-sm text-gray-400 mt-2">Beli asuransi terlebih dahulu</p>
                            </div>
                        ) : (
                                    ...
                                    >
                                        ...
                                        {activeInsurance.map((ins: any) => (
                                            <option key={ins.id} value={ins.id}>
                                                {ins.tracking_number} - {formatRupiah(ins.coverage_amount)}
                                            </option>
                                        ))}
                                    </select>
                                </div>
                ...
                {/* Claims History */}
                <div className="bg-white rounded-xl shadow-md p-6">
                    <h2 className="text-xl font-bold text-gray-900 mb-6">Riwayat Klaim</h2>
                                    ...
                                    <div className="flex items-start justify-between mb-3">
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-600">
                                                {claim.claim_type.toUpperCase()} - {claim.auto_detected ? '🤖 Auto-detected' : '📝 Manual'}
                                            </p>
                                        </div>
                                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${claim.status === 'approved' ? 'bg-green-100 text-green-800' :
                ...
                <div className="mt-8 bg-blue-50 border border-blue-200 rounded-xl p-6">
                    <h3 className="font-bold text-blue-900 mb-3">ℹ️ Tentang Asuransi Paket</h3>
                    <ul className="text-sm text-blue-800 space-y-2">
                        ...
                        <li>• Sistem auto-detect akan otomatis membuat klaim jika terdeteksi anomali</li>
