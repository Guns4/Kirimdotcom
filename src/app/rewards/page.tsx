...
export default function RewardsPage() {
  ...
  const handleRedeem = async (item: any) => {
    ...
    try {
      ...
      if (res.success) {
        toast.success('Berhasil ditukar!', {
          description: `Kode voucher Anda: ${res.code}`,
        });
      }
    } catch (e: any) {
    ...
    <div className="container-custom py-8 space-y-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500">
            Tukarkan poin aktivitasmu dengan hadiah menarik.
          </p>
        </div>
        <CoinBalance />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {rewards.map((item) => (
          <Card
            key={item.id}
            className="hover:shadow-lg transition-all border-indigo-50"
          >
            ...
            <CardFooter>
              <Button className="w-full" onClick={() => handleRedeem(item)}>
                Tukar Sekarang
              </Button>
            </CardFooter>
          </Card>
