...
bot.command('cek', async (ctx) => {
  ...
  if (!resi) {
    return ctx.reply(
      '⚠️ Mohon sertakan nomor resi.\nContoh: `/cek JP123456789`',
      {
        parse_mode: 'Markdown',
      }
    );
...
bot.command('ongkir', async (ctx) => {
  ...
  await ctx.reply(
    `🔍 Cek ongkir: ${origin.name} -> ${dest.name} (${weightKg}kg)...`
  );
  ...
  if (!result.success || !result.data) {
    return ctx.reply(
      '❌ Gagal mengambil data ongkir. Silakan coba lagi nanti.'
    );
  }
  ...
  replyMsg +=
    `[Lihat ${result.data.length} opsi lainnya di CekKirim.com](https://cekkirim.com)` +
    MARKETING_FOOTER;
