...
async function handleRequest(req: Request, { slug }: { slug: string[] }, method: string) {
    ...
    try {
        ...
        if (endpoint === 'shipping/cost' && method === 'POST') {
            ...
            const internalRes = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/shipping/cost`, {
                method: 'POST',
            ...
            const internalRes = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/tracking/${resiNumber}`);
            response = await internalRes.json();
        } else {
            return NextResponse.json(
        ...
        console.log(`[API Gateway] ${method} /v1/${endpoint} - ${responseTime}ms - User: ${keyData.users.email}`);
