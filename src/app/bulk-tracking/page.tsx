...
class TrackingQueue {
  ...
  private async trackSingle(resi: string): Promise<TrackingResult> {
    ...
    return {
      ...
      status:
        ...
        'Unknown',
      statusDate:
        trackingData.summary?.date || trackingData.history?.[0]?.date || '-',
      description:
        trackingData.summary?.desc || trackingData.status?.desc || '-',
      isError: false,
    };
  }
  ...
  private detectCourier(resi: string): string {
    ...
    if (
      resiUpper.startsWith('00') ||
      (resiUpper.length === 15 && resiUpper.match(/^\d+$/))
    )
      return 'sicepat';
    if (resiUpper.startsWith('10') && resiUpper.length >= 12) return 'anteraja';
    if (resiUpper.match(/^[A-Z]{3}\d+$/)) return 'jne';
    if (resiUpper.startsWith('NV') || resiUpper.startsWith('NI'))
      return 'ninja';
...
function BulkTrackingContent() {
  ...
  const [input, setInput] = useState(
    initialResi ? initialResi.split(',').join('\n') : ''
  );
  ...
  const handleExportCSV = () => {
    ...
    exportToCSV(
      getFormattedData(),
      columns,
      `tracking-results-${new Date().toISOString().split('T')[0]}`
    );
  };
  ...
  const handleCopyTable = async () => {
    ...
    if (success) {
      alert(
        'Data berhasil disalin! Silakan Paste (Ctrl+V) di Google Sheets/Excel.'
      );
    } else {
            ...
            {isProcessing && (
              <div className="mb-6">
                <div className="flex items-center justify-between text-sm text-gray-400 mb-2">
                  ...
                  <span>
                    {progress.current} dari {progress.total}
                  </span>
                </div>
