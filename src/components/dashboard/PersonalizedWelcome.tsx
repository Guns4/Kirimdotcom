...
const DAILY_TIPS = [
  {
    tip: 'Foto produk yang bagus bisa meningkatkan konversi hingga 40%!',
    category: 'Foto',
  },
  {
    tip: 'Respon chat dalam 5 menit pertama = 10x lebih mungkin closing.',
    category: 'Chat',
  },
  {
    tip: 'Packing rapi dengan bubble wrap mencegah komplain dan rating buruk.',
    category: 'Packing',
  },
  {
    tip: 'Kirim paket sebelum jam 2 siang agar bisa pickup hari ini.',
    category: 'Pengiriman',
  },
  {
    tip: 'Follow up pelanggan 3 hari setelah terima paket untuk minta review.',
    category: 'Review',
  },
  {
    tip: 'Gunakan resi instant untuk pelanggan yang urgent!',
    category: 'Resi',
  },
  {
    tip: 'Cek ongkir di 3 ekspedisi berbeda untuk hemat biaya.',
    category: 'Ongkir',
  },
  {
    tip: 'Update stok setiap hari untuk hindari "Maaf, Habis".',
    category: 'Stok',
  },
  {
    tip: 'Kasih bonus kecil (stiker/permen) = pelanggan ingat terus!',
    category: 'Branding',
  },
  {
    tip: 'Jangan lupa catat pengeluaran kecil seperti lakban dan bensin.',
    category: 'Keuangan',
  },
  {
    tip: 'Buat template chat untuk pertanyaan yang sering diulang.',
    category: 'Efisiensi',
  },
    ...
    <div className="bg-gradient-to-r from-primary-600 to-accent-500 rounded-2xl p-6 text-white">
      <div className="flex items-start justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold mt-1">{shopName || 'Seller'}!</h1>
        </div>
      ...
      {typeof pendingPackages === 'number' && pendingPackages > 0 && (
        <p className="mt-4 text-white/90">
          Ada <strong>{pendingPackages} paket</strong> yang belum dikirim hari
          ini.
        </p>
      )}
    </div>
...
export function QuickResume({
  ...
  lastTracking?: WelcomeData['lastTracking'];
}) {
    ...
    <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-surface-700 text-sm leading-relaxed">{tip.tip}</p>
        </div>
      </div>
    </div>
  );
