...
export function ComplaintButton({
  ...
  const generateEmail = () => {
    const subject = encodeURIComponent(
      `Komplain Paket Stuck - ${courier.toUpperCase()} - ${awb}`
    );
...
Terima kasih.`
    ...
    window.open(
      `mailto:customercare@${courier.toLowerCase()}.co.id?subject=${subject}&body=${body}`,
      '_blank'
    );
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h4 className="font-semibold text-red-900">
            Paket Terindikasi Stuck
          </h4>
          <p className="text-xs text-red-600">
            Tidak bergerak lebih dari 3 hari.
          </p>
        </div>
      </div>
      <Button
