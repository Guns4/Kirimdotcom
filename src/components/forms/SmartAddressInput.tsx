...
export function parseAddress(text: string): ParsedAddress {
  ...
  const kelMatch = text.match(
    /(?:Kel(?:urahan)?\.?\s*|Desa\s*)([A-Za-z\s]+?)(?:,|\n|Kec|RT|$)/i
  );
  if (kelMatch) {
  ...
  const streetMatch = text.match(
    /(?:Jl\.?|Jalan|Gg\.?|Gang|Komp\.?|Komplek|Blok)\s*[A-Za-z0-9\s.-]+/i
  );
  if (streetMatch) {
