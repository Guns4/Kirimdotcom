      ...
      try {
        ...
        if (useWatermark) {
          ...
          const watermarkedFile = new File(
            [await applyWatermark(outputBlob)],
            originalFile.name,
            {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="space-y-6">
        <div
          ...
          className={`glass-card p-8 border-2 border-dashed transition-all cursor-pointer flex flex-col items-center justify-center text-center min-h-[300px] ${
            originalFile
        ...
        >
          ...
          {originalFile ? (
            <div className="space-y-4">
              ...
              <div>
                <h3 className="text-white font-medium truncate max-w-[200px] mx-auto">
                  {originalFile.name}
                </h3>
            ...
            <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/5">
              ...
              <button
                ...
                className={`w-12 h-6 rounded-full transition-colors relative ${
                  useWatermark ? 'bg-indigo-600' : 'bg-gray-600'
                }`}
              >
                <div
                  className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${
                    useWatermark ? 'translate-x-6' : 'translate-x-0'
                  }`}
                />
              </button>
            </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4 text-center">
                <div>
                  ...
                  <p className="text-white font-mono">
                    {formatSize(originalFile?.size || 0)}
                  </p>
                </div>
                <div className="border-l border-white/10">
                  ...
                  <p className="text-green-400 font-mono font-bold flex items-center justify-center gap-1">
                    ...
                    <span className="text-[10px] bg-green-500/20 px-1.5 py-0.5 rounded text-green-300">
                      -
                      {Math.round(
                        (1 - compressedFile.size / (originalFile?.size || 1)) *
                          100
                      )}
                      %
                    </span>
