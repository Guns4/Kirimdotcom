...
export default function MarketplaceCalculator() {
  ...
  const tierOptions = useMemo(() => {
    ...
    return selectedPlatform.data.tiers.map((t: any) => ({
      value: t.id,
      label: t.name,
      data: t,
    }));
  ...
  const result = useMemo(() => {
    ...
    const fsFee = tier.freeShippingFee
      ? (valPrice * tier.freeShippingFee) / 100
      : 0;
  ...
  const chartData = useMemo(() => {
    ...
    return [
      ...
      {
        name: 'Est. Profit Bersih',
        value: result.netProfit > 0 ? result.netProfit : 0,
      },
        ...
        <div className="bg-gradient-to-r from-orange-500/10 to-yellow-500/10 border border-orange-500/20 p-4 rounded-xl flex items-start gap-3">
          ...
          <div>
            <p className="text-sm text-orange-200 font-medium">
              Mau margin lebih besar?
            </p>
            <p className="text-xs text-gray-400 mb-2">
              Kurangi biaya packing dengan supplier tangan pertama.
            </p>
            <a
            ...
            >
              Beli Plastik & Bubble Wrap Hemat{' '}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="glass-card p-4 border-l-4 border-indigo-500">
            ...
            <h3 className="text-lg font-bold text-white">
              Rp{' '}
              {result.totalDeduction.toLocaleString('id-ID', {
                maximumFractionDigits: 0,
              })}
            </h3>
            <p className="text-xs text-indigo-400 mt-1">
              ~
              {((result.totalDeduction / (Number(price) || 1)) * 100).toFixed(
                1
              )}
              % dari harga
            </p>
          ...
          <div
            className={`glass-card p-4 border-l-4 ${result.netProfit >= 0 ? 'border-emerald-500' : 'border-red-500'}`}
          >
            <p className="text-xs text-gray-400 mb-1">Profit Bersih</p>
            <h3
              className={`text-lg font-bold ${result.netProfit >= 0 ? 'text-emerald-400' : 'text-red-400'}`}
            >
              Rp{' '}
              {result.netProfit.toLocaleString('id-ID', {
                maximumFractionDigits: 0,
              })}
            </h3>
            <p
              className={`text-xs mt-1 ${result.netProfit >= 0 ? 'text-emerald-500' : 'text-red-500'}`}
            >
        ...
        {selectedTier && (
          <div className="glass-card p-4 space-y-2 text-sm">
            <h4 className="font-semibold text-white mb-3">
              Rincian Potongan {selectedPlatform.label}
            </h4>
            ...
            <div className="flex justify-between text-gray-300">
              <span>
                Biaya Layanan/Payment ({selectedTier.data.paymentFee}%)
              </span>
              <span>Rp {result.paymentFee.toLocaleString('id-ID')}</span>
            </div>
            <div className="flex justify-between text-gray-300">
              <span>
                Biaya Gratis Ongkir ({selectedTier.data.freeShippingFee || 0}%)
              </span>
