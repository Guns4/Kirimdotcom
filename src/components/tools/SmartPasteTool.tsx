...
export function SmartPasteTool() {
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      ...
      <div className="space-y-4">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Tempel chat kotor dari customer di sini. Kami akan ambil nomor
              resinya saja.
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <div className="space-y-4">
        <Card
          className={`h-full ${isCleaned ? 'bg-indigo-50 border-indigo-200' : 'bg-gray-50 border-dashed'}`}
        >
          <CardHeader>
              ...
              <div className="space-y-6">
                <div className="flex flex-wrap gap-2">
                  {extracted.map((resi, i) => (
                    <span
                      key={i}
                      className="px-3 py-1 bg-white border border-indigo-200 text-indigo-700 rounded-full text-sm font-mono shadow-sm"
                    >
                      {resi}
                    </span>
                  ))}
