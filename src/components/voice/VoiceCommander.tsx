...
import { useState } from 'react'; // useEffect removed since not strictly needed for basic setup hook usage here
  ...
  const commands = [
    {
      ...
      callback: (page: string) => {
        ...
        if (p.includes('beranda') || p.includes('home')) router.push('/');
        if (p.includes('cek resi')) router.push('/bulk-tracking');
        if (p.includes('dashboard')) router.push('/dashboard');
      },
    },
    {
      command: 'lacak *',
      callback: (resi: string) => {
        ...
        const cleanResi = String(resi).replace(/\s/g, '');
