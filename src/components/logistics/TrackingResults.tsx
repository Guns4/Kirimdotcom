...
const TrackingMap = dynamic(() => import('../tracking/TrackingMap'), {
  loading: () => (
    <div className="h-64 bg-gray-100 dark:bg-gray-800 animate-pulse rounded-xl flex items-center justify-center text-gray-400">
      Loading Map...
    </div>
  ),
  ssr: false,
...
export function TrackingResults({ result, onRetry }: TrackingResultsProps) {
  ...
  const isDelivered =
    data.currentStatus?.toUpperCase().includes('TERKIRIM') ||
  ...
  return (
    <div className="space-y-4">
      ...
      <motion.div
        ...
        className={
          isLiteMode
            ? 'bg-white p-5 border border-green-100 shadow-sm rounded-xl mb-6'
            : 'glass-card p-5 border-green-500/30 bg-gradient-to-r from-green-500/5 to-emerald-500/5'
        }
      >
      ...
      <motion.div
        ...
        className={
          isLiteMode
            ? 'bg-white p-4 mb-6 border border-purple-100 shadow-sm rounded-xl'
            : 'glass-card p-4 bg-gradient-to-r from-purple-500/10 to-indigo-500/10 border-purple-500/30'
        }
      >
      ...
      <motion.div
        ...
        className={
          isLiteMode
            ? 'bg-white p-6 border border-gray-200 shadow-sm rounded-xl'
            : 'glass-card p-6'
        }
      >
        ...
        <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl">
          <div>
            ...
            <div className="flex items-center gap-2">
              <span
                className={`px-3 py-1 rounded-full text-sm font-semibold ${getStatusColor(data.currentStatus)}`}
              >
          ...
          <>
            ...
            <div className="space-y-6 relative">
              ...
              {data.history.map((item, index) => {
                ...
                const isDelivered =
                  item.desc?.toUpperCase().includes('TERKIRIM') ||
                  item.desc?.toUpperCase().includes('DELIVERED');
                  ...
                  <div key={index} className="relative pl-16">
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        <h4
                          className={`font-semibold ${isLatest ? 'text-indigo-300' : 'text-white'}`}
                        >
