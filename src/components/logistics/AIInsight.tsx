...
export function AIInsight({ message, type = 'info', className = '' }: AIInsightProps) {
    const getIcon = () => {
        switch (type) {
            case 'success':
                return <Sparkles className="w-5 h-5 text-green-400 mt-0.5 flex-shrink-0" />
            case 'warning':
                return <AlertTriangle className="w-5 h-5 text-yellow-400 mt-0.5 flex-shrink-0" />
            case 'tip':
                return <Lightbulb className="w-5 h-5 text-blue-400 mt-0.5 flex-shrink-0" />
            default:
                return <Sparkles className="w-5 h-5 text-purple-400 mt-0.5 flex-shrink-0" />
        }
    }
