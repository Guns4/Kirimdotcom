...
export default function WARotatorForm({ onSubmit }: WARotatorFormProps) {
    ...
    const [defaultMessage, setDefaultMessage] = useState('Halo! Saya tertarik dengan produk Anda.');
    const [isSubmitting, setIsSubmitting] = useState(false);

    const handleAddCS = () => {
        setCSNumbers([...csNumbers, { number: '', name: `CS ${csNumbers.length + 1}` }]);
    };
    ...
    const handleCSChange = (index: number, field: 'number' | 'name', value: string) => {
                    ...
                    {csNumbers.map((cs, index) => (
                        <div key={index} className="flex items-center gap-3">
                            ...
                            <input
                                type="tel"
                                value={cs.number}
                                onChange={(e) => handleCSChange(index, 'number', e.target.value)}
