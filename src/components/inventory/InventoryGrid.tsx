...
import { Plus, Minus, Package, AlertTriangle, AlertCircle, Search } from 'lucide-react';
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-white p-4 rounded-xl border shadow-sm">
                    <div className="text-gray-500 text-xs font-bold uppercase tracking-wider">Total Produk</div>
                    <div className="text-2xl font-black text-gray-800">{items.length}</div>
                </div>
                <div className="bg-white p-4 rounded-xl border shadow-sm">
                    <div className="text-gray-500 text-xs font-bold uppercase tracking-wider">Total Stok</div>
                    <div className="text-2xl font-black text-indigo-600">
                        {items.reduce((acc, curr) => acc + curr.stock, 0)}
                    </div>
                </div>
                <div className={`p-4 rounded-xl border shadow-sm ${lowStockCount > 0 ? 'bg-orange-50 border-orange-200' : 'bg-white'}`}>
                    <div className={`${lowStockCount > 0 ? 'text-orange-600' : 'text-gray-500'} text-xs font-bold uppercase tracking-wider flex items-center gap-1`}>
                        {lowStockCount > 0 && <AlertTriangle className="w-3 h-3" />}
                        Stok Menipis
                    </div>
                    <div className={`text-2xl font-black ${lowStockCount > 0 ? 'text-orange-700' : 'text-gray-800'}`}>
                        {lowStockCount}
                    </div>
                </div>
                <div className={`p-4 rounded-xl border shadow-sm ${outOfStockCount > 0 ? 'bg-red-50 border-red-200' : 'bg-white'}`}>
                    <div className={`${outOfStockCount > 0 ? 'text-red-600' : 'text-gray-500'} text-xs font-bold uppercase tracking-wider flex items-center gap-1`}>
                        {outOfStockCount > 0 && <AlertCircle className="w-3 h-3" />}
                        Habis
                    </div>
                    <div className={`text-2xl font-black ${outOfStockCount > 0 ? 'text-red-700' : 'text-gray-800'}`}>
                        {outOfStockCount}
                    </div>
                </div>
            </div>
                ...
                {filtered.map(item => (
                    ...
                    >
                        ...
                        <div className="p-4 flex-1 flex flex-col">
                            ...
                            <h3 className="font-bold text-gray-800 leading-tight mb-2 line-clamp-2" title={item.name}>
                                {item.name}
                            </h3>

                            <div className="mt-auto pt-4 border-t border-gray-50 flex items-center justify-between">
                                <div>
                                    <div className="text-[10px] text-gray-400 uppercase">Harga Jual</div>
