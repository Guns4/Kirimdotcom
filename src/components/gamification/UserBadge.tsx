...
export function UserBadge({
  ...
  if (points >= 10000) {
    ...
    Icon = Gem;
    colorClass =
      'text-cyan-300 bg-cyan-400/10 border-cyan-400/30 shadow-[0_0_15px_rgba(34,211,238,0.3)]';
    ...
    shineEffect =
      'relative overflow-hidden before:absolute before:top-0 before:-left-[150%] before:w-[50%] before:h-full before:bg-gradient-to-r before:from-transparent before:via-white/50 before:to-transparent before:skew-x-[30deg] before:animate-[shimmer_2s_infinite]';
  } else if (points >= 1000) {
    tier = 'Silver';
    Icon = Star;
  ...
  // Note: 'shimmer' keyframe has been added to tailwind.config.ts by Antigravity.
  // if (tier === 'Gold') shineEffect = ''; // Reset complex effect for stability, use standard glow <-- Commenting this out since we added support!
  ...
  return (
    ...
    >
      <Icon
        className={`w-3.5 h-3.5 fill-current ${tier === 'Platinum' ? 'animate-spin-slow' : ''}`}
      />
