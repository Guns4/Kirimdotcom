...
export function RankBadge({
  ...
  return (
    <div className={`inline-flex items-center gap-1.5 ${className}`}>
      {/* Rank Icon */}
      <span className={`${sizeClasses[size]} ${rank.color}`} title={rank.name}>
        {rank.icon}
      </span>
            ...
            {rankChanged ? (
              <>
                ...
                <p className="text-sm opacity-90">
                  Selamat! Anda naik ke{' '}
                  <span className="font-semibold">{newRank}</span>
                </p>
              </>
            ) : (
              <>
    ...
    <div className={`glass-card p-6 ${className}`}>
      ...
      {isPremium && premiumExpiresAt && (
        <p className="mt-3 text-xs text-yellow-400/80">
          Premium berakhir:{' '}
          {new Date(premiumExpiresAt).toLocaleDateString('id-ID')}
        </p>
      )}
...
export function MiniBadge({
  ...
  return (
    ...
    >
      <button
        className={`flex items-center gap-1 px-2 py-1 rounded-lg ${rank.bgColor} transition-colors hover:opacity-80`}
      >
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p
                  className={`font-medium truncate ${isCurrentUser ? 'text-indigo-300' : 'text-white'}`}
                >
                  {entry.display_name || 'Anonymous'}
                  {isCurrentUser && (
                    <span className="text-xs text-gray-500 ml-1">(Anda)</span>
                  )}
                </p>
