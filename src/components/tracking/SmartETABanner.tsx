...
export function SmartETABanner({
  courier,
  origin,
  dest,
}: {
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-white/90 mt-1">
            Berdasarkan {prediction.confidence_score} pengiriman sebelumnya,
            paket diprediksi sampai:
          </p>
          <div className="mt-2 flex items-center gap-2 font-mono text-lg font-bold bg-black/20 w-fit px-3 py-1 rounded">
            <Clock className="w-4 h-4" />
            {new Date(prediction.estimated_arrival).toLocaleDateString(
              undefined,
              {
                weekday: 'long',
                day: 'numeric',
