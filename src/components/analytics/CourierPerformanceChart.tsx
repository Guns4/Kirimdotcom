        ...
        <div className="space-y-6">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Data dikumpulkan dari {data.reduce((a, b) => a + b.sample_size, 0)} paket yang berhasil terkirim.
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="h-[400px] w-full mt-4">
                        <ResponsiveContainer width="100%" height="100%">
                            ...
                            >
                                ...
                                />
                                <ReferenceLine x={3} stroke="#9ca3af" strokeDasharray="3 3" label={{ position: 'top', value: '3 Hari (Batang Standar)' }} />
                                <Bar dataKey="avg_duration_days" radius={[0, 4, 4, 0]} barSize={32}>
                                    {data.map((entry, index) => (
                                        <Cell
                                            key={`cell-${index}`}
                                            fill={
                                                entry.avg_duration_days < 2.5 ? COLORS.fast :
                                                    entry.avg_duration_days < 4 ? COLORS.medium : COLORS.slow
                                            }
                                        />
                                    ))}
                                </Bar>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center text-sm text-muted-foreground">
                <div className="flex items-center justify-center gap-2">
                    <div className="w-3 h-3 rounded-full" style={{ background: COLORS.fast }} />
                    <span>Cepat (&lt; 2.5 Hari)</span>
                </div>
                <div className="flex items-center justify-center gap-2">
                    <div className="w-3 h-3 rounded-full" style={{ background: COLORS.medium }} />
                    <span>Standar (2.5 - 4 Hari)</span>
                </div>
                <div className="flex items-center justify-center gap-2">
                    <div className="w-3 h-3 rounded-full" style={{ background: COLORS.slow }} />
