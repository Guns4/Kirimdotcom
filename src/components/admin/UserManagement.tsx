...
export function UserManagement() {
  ...
  const handleBanUser = async (userId: string) => {
    ...
    if (result.success) {
      ...
      setToast({
        message: result.error || 'Gagal memblokir user',
        type: 'error',
      });
    }
  ...
  const handleUpgradeToPremium = async (userId: string) => {
    ...
    if (result.success) {
      setToast({
        message: 'User berhasil di-upgrade ke Premium',
        type: 'success',
      });
      fetchUsers();
    } else {
      setToast({
        message: result.error || 'Gagal upgrade user',
        type: 'error',
      });
    }
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/10">
                ...
                <th className="text-right py-4 px-6 text-gray-400 text-sm font-medium">
                  Aksi
                </th>
              </tr>
            </thead>
