    ...
    return (
        <div className="bg-white border rounded-xl p-4 shadow-sm">
            ...
            <div className="grid grid-cols-3 gap-2">
                {/* LCP */}
                <div className={`p-3 rounded-lg border text-center ${colors[getStatus('lcp', dummyData.lcp)]}`}>
                    <div className="text-xs font-semibold opacity-80">LCP</div>
                    <div className="font-bold text-lg">{(dummyData.lcp / 1000).toFixed(1)}s</div>
                </div>

                {/* CLS */}
                <div className={`p-3 rounded-lg border text-center ${colors[getStatus('cls', dummyData.cls)]}`}>
                    <div className="text-xs font-semibold opacity-80">CLS</div>
                    <div className="font-bold text-lg">{dummyData.cls.toFixed(2)}</div>
                </div>

                {/* INP */}
                <div className={`p-3 rounded-lg border text-center ${colors[getStatus('inp', dummyData.inp)]}`}>
