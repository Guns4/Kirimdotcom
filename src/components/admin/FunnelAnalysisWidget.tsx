...
export default function FunnelAnalysisWidget() {
  ...
  if (!stats)
    return (
      <div className="p-6 bg-white rounded-xl border animate-pulse h-64"></div>
    );
  ...
  const funnel = [
    ...
    {
      label: 'Interest',
      value: stats.view_register_page || 0,
      icon: Users,
      color: 'bg-indigo-100 text-indigo-600',
    },
    {
      label: 'Registered',
      value: stats.complete_registration || 0,
      icon: UserPlus,
      color: 'bg-purple-100 text-purple-600',
    },
    {
      label: 'Paid Users',
      value: stats.complete_purchase || 0,
      icon: CreditCard,
  ...
  const totalConversion = calculateConversion(
    stats.complete_purchase,
    stats.view_landing_page
  );
      ...
      <div className="space-y-2">
        {funnel.map((step, i) => {
          ...
          const ratio =
            i === 0
              ? 100
              : parseFloat(calculateConversion(step.value, prevValue));
                ...
                <div className="text-right">
                  ...
                  {i > 0 && (
                    <div className="text-xs font-semibold text-gray-400 bg-white/50 px-1 rounded inline-flex items-center">
                      {ratio}%{' '}
                      <ArrowRight className="w-3 h-3 ml-0.5 rotate-45" />
                    </div>
                  )}
                </div>
              </div>
