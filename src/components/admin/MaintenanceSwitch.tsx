...
export function MaintenanceSwitch({ initialStatus }: MaintenanceSwitchProps) {
    ...
    const handleToggle = async () => {
        ...
        try {
            ...
            toast.success(isMaintenance ? 'Website Online Kembali' : 'Mode Darurat Aktif')
        } catch (error) {
    ...
    return (
        <div className={`p-6 rounded-xl border transition-all ${isMaintenance ? 'bg-red-500/10 border-red-500/30' : 'glass-card border-white/10'}`}>
            <div className="flex items-start justify-between">
                <div className="flex gap-4">
                    <div className={`p-3 rounded-xl ${isMaintenance ? 'bg-red-500/20' : 'bg-white/5'}`}>
                        <Power className={`w-6 h-6 ${isMaintenance ? 'text-red-400' : 'text-gray-400'}`} />
                    </div>
                    <div>
                        ...
                        <p className="text-sm text-gray-400 mt-1 max-w-sm">
                            Matikan akses website sementara jika terjadi bug kritis atau serangan.
                        </p>
                    </div>
                </div>
                ...
                <button
                    ...
                    disabled={isLoading}
                    className={`relative w-14 h-8 rounded-full transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 focus:ring-red-500 ${isMaintenance ? 'bg-red-600' : 'bg-gray-600'
                        }`}
                >
                    <motion.div
                        initial={false}
            ...
            {isMaintenance && (
                <div className="mt-4 flex items-center gap-2 text-sm text-red-300 bg-red-500/10 p-3 rounded-lg border border-red-500/20">
                    ...
                    <span className="font-semibold">Website sedang dalam mode perbaikan!</span>
                </div>
            )}
        </div>
    )
