...
export function CourierLeaderboard({ title, description, stats, period }: CourierLeaderboardProps) {
    // Chart data
        ...
        <div className="glass-card p-6">
            ...
            {stats.length > 0 ? (
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-white/10">
                                    ...
                                    <th className="text-left py-3 px-4 text-sm font-semibold text-gray-400">Kurir</th>
                                    <th className="text-center py-3 px-4 text-sm font-semibold text-gray-400">Rating</th>
                                    <th className="text-center py-3 px-4 text-sm font-semibold text-gray-400">Review</th>
                                    <th className="text-center py-3 px-4 text-sm font-semibold text-gray-400">Sentiment</th>
                                </tr>
                            </thead>
                            <tbody>
                                {stats.map((stat, index) => {
                                    ...
                                    return (
                                        <tr key={stat.courier_code} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                            {/* Rank */}
                                            ...
                                            <td className="py-4 px-4">
                                                <div>
                                                    <p className="text-white font-medium">{stat.courier_name || stat.courier_code}</p>
                                                    {stat.courier_name && (
                                                        <p className="text-xs text-gray-500">{stat.courier_code}</p>
                                                    )}
                                                </div>
                                            </td>
                                            ...
                                            <td className="py-4 px-4 text-center">
                                                <div className="flex items-center justify-center gap-1">
                                                    ...
                                                    <span className="text-white font-semibold">{rating.toFixed(2)}</span>
                ...
                <div className="text-center py-12">
                    ...
                    <p className="text-gray-400">Belum ada data review untuk periode ini</p>
                    <p className="text-sm text-gray-500 mt-1">Jadilah yang pertama memberikan review!</p>
                </div>
            )}
        </div>
    )
