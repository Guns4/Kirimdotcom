...
import { searchRegions, getRegionLabel, type Region } from '@/data/indonesia-regions'
                ...
                <div className="absolute z-50 w-full mt-2 bg-slate-800 border border-white/10 rounded-xl shadow-xl max-h-64 overflow-y-auto">
                    {results.map((region, index) => (
                        <button
                            ...
                            className={`w-full flex items-start gap-3 px-4 py-3 text-left transition-colors ${index === highlightedIndex
                                ? 'bg-indigo-600/20 text-indigo-300'
                                : 'hover:bg-white/5 text-white'
                                }`}
                            type="button"
                        >
                            <MapPin className={`w-4 h-4 mt-0.5 flex-shrink-0 ${index === highlightedIndex ? 'text-indigo-400' : 'text-gray-500'
                                }`} />
                            <div className="min-w-0">
                                <p className="font-medium text-sm truncate">{region.district}</p>
                                <p className="text-xs text-gray-400 truncate">
