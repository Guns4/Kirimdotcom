...
export function LocalAdBanner({
    ...
    useEffect(() => {
        if (displayAd && isVisible) {
            ...
            console.log(`[Ad Impression] ${displayAd.id} for: ${destinationCity || 'default'}`)
        }
    ...
    if (variant === 'compact') {
        return (
            <div
                className={`relative overflow-hidden rounded-xl transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                    } ${className}`}
            >
                <div className={`bg-gradient-to-r ${displayAd.bgColor} p-4`}>
                    ...
                    <p className="text-white font-semibold text-sm mb-1">{displayAd.title}</p>
    ...
    if (variant === 'inline') {
        return (
            <div
                className={`relative overflow-hidden rounded-lg transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                    } ${className}`}
            >
                <div className={`bg-gradient-to-r ${displayAd.bgColor} p-3 flex items-center gap-3`}>
                    <div className="flex-1 min-w-0">
                        <p className="text-white font-medium text-sm truncate">{displayAd.title}</p>
                        <p className="text-white/70 text-xs truncate">{displayAd.description}</p>
                    </div>
    ...
    return (
        <div
            className={`relative overflow-hidden rounded-2xl transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                } ${className}`}
        >
            {/* Background gradient */}
...
export function SmartAdPlacement({ position, destinationCity, destinationProvince }: AdPlacementProps) {
    const variant = position === 'sidebar' ? 'compact' : position === 'inline' ? 'inline' : 'large'
