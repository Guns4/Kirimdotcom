...
import { Printer, Download, Save, Trash2, Tag, Copy, ExternalLink, ShoppingBag } from 'lucide-react'
...
const COURIERS = [
    'JNE', 'J&T', 'SiCepat', 'AnterAja', 'ID Express',
    'Ninja Xpress', 'Lion Parcel', 'POS Indonesia', 'Wahana', 'GoSend', 'GrabExpress'
]
...
export function LabelGenerator() {
    ...
    const generatePDF = async () => {
        ...
        try {
            ...
            doc.setFontSize(11)
            // Multi-line address
            const splitAddress = doc.splitTextToSize(data.receiver.address || '-', 90)
    ...
    const handleReceiverChange = (field: keyof typeof data.receiver, value: string) => {
            ...
            <div className="glass-card p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-400 mt-1">
                        Buat label pengiriman standar A6 (100x150mm) siap cetak. Gratis & tanpa batas.
                    </p>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="glass-card p-6 space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-medium text-gray-400 mb-1">Nama Toko/Pengirim</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="block text-xs font-medium text-gray-400 mb-1">No. HP</label>
                                <input
                                    type="tel"
                    ...
                    <div className="glass-card p-6 space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-medium text-gray-400 mb-1">Nama Penerima</label>
                                <input
                                    type="text"
                    ...
                    <div className="glass-card p-6 space-y-4">
                        <h3 className="font-semibold text-white flex items-center gap-2">
                            <div className="w-6 h-6 rounded-full bg-purple-500/20 text-purple-400 flex items-center justify-center text-xs">3</div>
                            Detail Paket
                        </h3>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-medium text-gray-400 mb-1">Ekspedisi</label>
                                <select
                                    value={data.details.courier}
                                    onChange={(e) => handleDetailsChange('courier', e.target.value)}
                            ...
                            <div>
                                <label className="block text-xs font-medium text-gray-400 mb-1">No. Resi/Booking (Opsional)</label>
                                <input
                                    type="text"
                            ...
                            <div className="md:col-span-1">
                                <label className="block text-xs font-medium text-gray-400 mb-1">Berat (kg)</label>
                                <input
                                    type="number"
                                    step="0.1"
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-xs font-medium text-gray-400 mb-1">Catatan (Fragile/Isi Paket)</label>
                                <input
                                    type="text"
                    ...
                    <div className="sticky top-24 space-y-6">
                        ...
                        <div className="bg-white rounded-lg p-4 shadow-lg aspect-[2/3] relative overflow-hidden flex flex-col text-black">
                            <div className="flex justify-between items-start border-b-2 border-black pb-2 mb-2">
                                <h2 className="text-2xl font-bold uppercase">{data.details.courier}</h2>
                            ...
                            {data.details.resi && (
                                <div className="h-16 bg-gray-100 mb-2 flex items-center justify-center border border-dashed border-gray-400">
                                    <span className="font-mono text-sm tracking-widest">{data.details.resi} (BARCODE)</span>
                                </div>
                            )}

                            <div className="flex-1 space-y-4">
                                <div>
                                    <div className="text-[10px] font-bold uppercase text-gray-500">Penerima</div>
                                    <div className="font-bold text-lg leading-tight">{data.receiver.name || 'Nama Penerima'}</div>
                                    <div className="text-sm">{data.receiver.phone || '08...'}</div>
                                    <div className="text-xs mt-1 leading-snug">{data.receiver.address || 'Alamat lengkap...'}</div>
                                </div>

                                <div className="border-t border-dashed border-gray-300 pt-2">
                                    <div className="text-[10px] font-bold uppercase text-gray-500">Pengirim</div>
                                    <div className="font-medium">{data.sender.name || 'Nama Toko'}</div>
                        ...
                        <div className="glass-card p-4 border border-yellow-500/20 bg-yellow-500/5">
                            ...
                            <div className="space-y-3">
                                ...
                                >
                                    <div>
                                        <div className="text-sm font-medium text-white group-hover:text-yellow-400 transition-colors">Kertas Thermal A6 Murah</div>
                                        <div className="text-xs text-gray-500">Mulai Rp 15.000/roll</div>
                                    </div>
                                ...
                                >
                                    <div>
                                        <div className="text-sm font-medium text-white group-hover:text-yellow-400 transition-colors">Printer Thermal Bluetooth</div>
                                        <div className="text-xs text-gray-500">Cetak resi dari HP tanpa tinta</div>
                                    </div>
                                    <ExternalLink className="w-4 h-4 text-gray-500" />
                                </a>
                            </div>
