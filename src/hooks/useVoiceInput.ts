...
export function useVoiceInput(onSmartMatch?: (result: SmartResult) => void): VoiceInputResult {
        ...
        if (typeof window !== 'undefined') {
            ...
            if (SpeechRecognition) {
                ...
                recognition.onerror = (event: any) => {
                    ...
                    setError(event.error === 'not-allowed' ? 'Izin mikrofon ditolak' : 'Gagal mengenali suara')
                    setIsListening(false)
                }
    ...
    const parseSmartInputs = (text: string) => {
        ...
        const match = text.replace(/\s/g, '').match(/([A-Z]*\d+[A-Z]*){6,}/i) || text.match(/[A-Z0-9]{8,}/i)
