    ...
    const validateField = useCallback(
        (name: keyof T): string | null => {
            try {
                ...
                const fieldSchema = (schema as z.ZodObject<z.ZodRawShape>).pick({ [name]: true } as Record<string, true>);
                fieldSchema.parse({ [name]: values[name] });
                return null;
            } catch (error) {
                if (error instanceof ZodError) {
                    const fieldError = (error as any).errors.find((e: any) => e.path[0] === name);
