  ...
  const addSearch = useCallback(
    (awb: string, courier?: string) => {
      setSearches((prev) => {
        ...
        const updated = [
          { awb, courier, timestamp: Date.now() },
          ...filtered,
        ].slice(0, maxItems);
...
export function useSavedAddresses() {
  ...
  const saveAddress = useCallback(
    (address: Omit<SavedAddress, 'id' | 'usageCount' | 'lastUsed'>) => {
      setAddresses((prev) => {
  ...
  const frequentAddresses = [...addresses].sort(
    (a, b) => b.usageCount - a.usageCount
  );

  // Sort by last used
  const recentAddresses = [...addresses].sort(
...
export function useUserPreferences() {
  ...
  const setDefaultCourier = useCallback(
    (courier: string) => {
  ...
  const setDefaultOriginCity = useCallback(
    (city: string) => {
...
export function useTrackingHistory(maxItems = 20) {
  ...
  const addToHistory = useCallback(
    (item: Omit<TrackingHistoryItem, 'lastChecked'>) => {
      setHistory((prev) => {
  ...
  return (
    <div className={className}>
      <div className="flex items-center justify-between mb-2">
        <h4 className="text-sm font-medium text-surface-600">
          🕐 Resi Terakhir
        </h4>
        <button
