...
`;
    ...
    const completion = await openai.chat.completions.create({
        messages: [
            {
                role: "system",
                content: "You are an expert Indonesian Logistic and E-commerce SEO Content Writer. You write engaging, informative, and SEO-optimized content that helps sellers and online shoppers."
            },
...
export async function generateMetaDescription(content: string): Promise<string> {
    ...
    const completion = await openai.chat.completions.create({
        messages: [
            {
                role: "system",
                content: "Generate a compelling SEO meta description (max 155 characters) in Indonesian."
            },
            {
                role: "user",
...
export async function suggestKeywords(topic: string): Promise<string[]> {
    ...
    const completion = await openai.chat.completions.create({
        messages: [
            {
                role: "system",
                content: "Suggest 5-8 relevant SEO keywords for Indonesian e-commerce/logistics articles. Return as comma-separated list."
            },
