...
const COLUMN_PATTERNS: Record<string, RegExp[]> = {
  ...
  customerPhone: [/no\.?\s*(telepon|hp|telp)/i, /phone/i, /telepon/i],
  customerAddress: [
  ...
  productQty: [/jumlah(\s*produk)?/i, /quantity/i, /qty/i],
  productPrice: [/harga\s*(awal|jual)?/i, /price/i, /unit\s*price/i],
  total: [
  ...
  for (const row of rows) {
    ...
    if (mapping.productName && row[mapping.productName]) {
      products.push({
        ...
        price:
          parseFloat(
            String(row[mapping.productPrice || '']).replace(/[^\d.-]/g, '')
          ) || 0,
      });
    }
    ...
    orders.push({
      ...
      orderDate:
        orderDate instanceof Date && !isNaN(orderDate.getTime())
          ? orderDate
          : undefined,
      customerName: row[mapping.customerName || '']
        ? String(row[mapping.customerName || ''])
        : undefined,
      customerPhone: row[mapping.customerPhone || '']
        ? String(row[mapping.customerPhone || ''])
        : undefined,
      customerAddress: row[mapping.customerAddress || '']
        ? String(row[mapping.customerAddress || ''])
        : undefined,
      products,
      totalAmount: total,
      courier: row[mapping.courier || '']
        ? String(row[mapping.courier || ''])
        : undefined,
