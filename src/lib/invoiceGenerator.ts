...
export function generateInvoicePDF(data: InvoiceData): jsPDF {
    ...
    if (data.shipping) {
        ...
        doc.setTextColor(textColor);
        doc.text(formatRupiah(data.shipping), pageWidth - 20, y, { align: 'right' });
        y += 7;
    }
    ...
    if (data.discount) {
        ...
        doc.text(`-${formatRupiah(data.discount)}`, pageWidth - 20, y, { align: 'right' });
        y += 7;
    }
    ...
    if (data.signatureImage) {
        try {
            doc.addImage(data.signatureImage, 'PNG', pageWidth - 65, signatureY + 25, 35, 20);
        } catch (e) {
            // Signature failed
        }
    }
    ...
    doc.setFontSize(9);
    doc.setTextColor(grayColor);
    doc.text(data.sellerName, pageWidth - 50, signatureY + 57, { align: 'center' });

    // ============ FOOTER ============
    const footerY = doc.internal.pageSize.getHeight() - 15;
    doc.setFontSize(8);
    doc.setTextColor(grayColor);
    doc.text('Invoice ini dibuat secara otomatis oleh CekKirim.com', pageWidth / 2, footerY, { align: 'center' });
